/*
 * Director dashboard script:
 * - Loads cross-branch sales, credits, stock, users, and branches.
 * - Computes dashboard totals and alerts.
 * - Manages director-only user and branch administration actions.
 */
...
function closeBranchModal(){document.getElementById("branchModal")?.remove()}
async function openAddBranchModal(){
  closeBranchModal();
  document.body.insertAdjacentHTML('beforeend',`<div class="custom-confirm" id="branchModal" style="z-index:10000;"><div class="confirm-box branch-modal-box"><h3>Add Branch</h3><form id="branchForm" class="branch-modal-form"><div class="form-group"><label for="branchName">Branch Name</label><input id="branchName" placeholder="Branch name" required></div><div class="form-group"><label for="branchLocation">Location</label><input id="branchLocation" placeholder="Location" required></div><div class="form-group"><label for="branchContact">Office Contact</label><input id="branchContact" placeholder="Contact" required></div><div class="form-group"><label for="branchEmail">Office Email</label><input id="branchEmail" type="email" placeholder="Email"></div><div id="branchFormError" class="branch-form-error"></div><div class="form-actions"><button type="button" class="btn-cancel" onclick="closeBranchModal()">Cancel</button><button type="submit" class="btn-primary">Create</button></div></form></div></div>`);
  document.getElementById('branchForm')?.addEventListener('submit',async(e)=>{
    e.preventDefault();
    const name=g('branchName')?.value?.trim();
    const location=g('branchLocation')?.value?.trim();
    const contact=g('branchContact')?.value?.trim();
    const email=g('branchEmail')?.value?.trim();
    try{await api(`${API_BASE}/branches`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,location,contact,email})});closeBranchModal();t('Branch created','success');await load()}catch(err){const e1=g('branchFormError');if(e1)e1.textContent=err.message;t(err.message,'error');}}
  );
}
async function openEditBranchModal(id){
  const b=allBranches.find(x=>String(x._id)===String(id));if(!b)return;
  closeBranchModal();
  document.body.insertAdjacentHTML('beforeend',`<div class="custom-confirm" id="branchModal" style="z-index:10000;"><div class="confirm-box branch-modal-box"><h3>Edit Branch</h3><form id="branchForm" class="branch-modal-form"><div class="form-group"><label for="branchName">Branch Name</label><input id="branchName" value="${escAttr(b.name||'')}" required></div><div class="form-group"><label for="branchLocation">Location</label><input id="branchLocation" value="${escAttr(b.location||'')}" required></div><div class="form-group"><label for="branchContact">Office Contact</label><input id="branchContact" value="${escAttr(b.contact||'')}" required></div><div class="form-group"><label for="branchEmail">Office Email</label><input id="branchEmail" type="email" value="${escAttr(b.email||'')}"></div><div id="branchFormError" class="branch-form-error"></div><div class="form-actions"><button type="button" class="btn-cancel" onclick="closeBranchModal()">Cancel</button><button type="submit" class="btn-primary">Save</button></div></form></div></div>`);
  document.getElementById('branchForm')?.addEventListener('submit',async(e)=>{
    e.preventDefault();
    const name=g('branchName')?.value?.trim();
    const location=g('branchLocation')?.value?.trim();
    const contact=g('branchContact')?.value?.trim();
    const email=g('branchEmail')?.value?.trim();
    try{await api(`${API_BASE}/branches/${id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,location,contact,email})});closeBranchModal();t('Branch updated','success');await load()}catch(err){const e1=g('branchFormError');if(e1)e1.textContent=err.message;t(err.message,'error');}}
  );
}
async function toggleBranchStatus(id){const b=allBranches.find(x=>String(x._id)===String(id));if(!b)return;const status=(b.status||'active')==='active'?'inactive':'active';try{await api(`${API_BASE}/branches/${id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({status})});t(`Branch ${status==='inactive'?'deactivated':'activated'} successfully`,'success');await load()}catch(e){t(e.message,'error')}}
let pendingBranchDeleteId=null;
function showDeleteBranchConfirm(id){const b=allBranches.find(x=>String(x._id)===String(id));if(!b)return;const k=nB(b.name);if(k==='maganjo'||k==='matugga'){t('Default branches cannot be deleted. Deactivate instead.','error');return}pendingBranchDeleteId=id;const m='deleteBranchConfirm';document.getElementById(m)?.remove();document.body.insertAdjacentHTML('beforeend',`<div class="custom-confirm" id="${m}" style="z-index:10000;"><div class="confirm-dialog"><i class="bi bi-exclamation-triangle-fill"></i><h3>Delete Branch</h3><p>Are you sure you want to delete <strong>${esc(b.name)}</strong>?</p><div class="confirm-actions"><button class="confirm-btn no" onclick="hideDeleteBranchConfirm()">No, Keep</button><button class="confirm-btn yes" onclick="confirmDeleteBranch()">Yes, Delete</button></div></div></div>`)}
function hideDeleteBranchConfirm(){document.getElementById('deleteBranchConfirm')?.remove();pendingBranchDeleteId=null}
async function confirmDeleteBranch(){if(!pendingBranchDeleteId)return;const id=pendingBranchDeleteId;hideDeleteBranchConfirm();try{await api(`${API_BASE}/branches/${id}`,{method:'DELETE'});t('Branch deleted successfully','success');await load()}catch(e){t(e.message,'error')}}
function deleteBranch(id){showDeleteBranchConfirm(id)}
function csv(rows){return rows.map(r=>r.map(v=>{const s=String(v??'');return(s.includes(',')||s.includes('"')||s.includes('\n'))?`"${s.replace(/"/g,'""')}"`:s}).join(',')).join('\n')}
function dl(name,text){const b=new Blob([text],{type:'text/csv;charset=utf-8;'});const u=URL.createObjectURL(b);const a=document.createElement('a');a.href=u;a.download=name;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(u)}
...
function notif(){const a=getDirectorAlerts();updateNotifBadge(a.low.length+a.out.length+a.overdue.length)}
...
function confirmLogout(){hideLogoutConfirm();localStorage.removeItem('token');localStorage.removeItem('user');t('Logged out successfully','success');setTimeout(()=>window.location.href='../../index.html',1500)}
window.openAddUserModal=openAddUserModal;window.openEditUserModal=openEditUserModal;window.toggleUserStatus=toggleUserStatus;window.deleteUser=deleteUser;window.showDeleteUserConfirm=showDeleteUserConfirm;window.hideDeleteUserConfirm=hideDeleteUserConfirm;window.confirmDeleteUser=confirmDeleteUser;window.closeUserModal=closeUserModal;
window.openAddBranchModal=openAddBranchModal;window.openEditBranchModal=openEditBranchModal;window.toggleBranchStatus=toggleBranchStatus;window.deleteBranch=deleteBranch;window.showDeleteBranchConfirm=showDeleteBranchConfirm;window.hideDeleteBranchConfirm=hideDeleteBranchConfirm;window.confirmDeleteBranch=confirmDeleteBranch;window.closeUserModal=closeUserModal;window.closeBranchModal=closeBranchModal;
